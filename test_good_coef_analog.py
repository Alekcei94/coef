...
def coefficients():
    xlist = [-60, -26, -1, 33, 72, 123]
    ### TYPICAL CASE
    ylist = [61450, 61934, 62224, 62552, 62864, 63191]

    print(str(len(xlist)) + " __ " + str(len(ylist)))
    xlistNew = xlist
    ylistNew = []
...
def calculationOfCoefficients(xlist, ylist, all_minus):
    ...
    plt.plot([-60, 120], [348, 3655], color='red')
    plt.show()
    '''
    # NEW BLOCK
    print("START")
    ...
    for iterator in range(len(coefB)):
        coefB[iterator] = coefB[iterator] - 300
    list_b_new = []
    for ele in coefB:
        x = ele
